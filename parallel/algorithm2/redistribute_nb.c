...
#include "redistribute_nb.h"

double ** redistribute_nb(int N,
                       int M,
                       double **A_in,
                       int nblock_in_r,
                       int nblock_in_c,
    ...
    if (rank < p_in_r*p_in_c) {
        N_local_in_r = N; //local_size(N, nblock_in_r, rank, p_in_r, p_in_c, 'r');
        N_local_in_c = M; //local_size(N, nblock_in_c, rank, p_in_c, p_in_r, 'c');
    }
    else {
        N_local_in_r = 0;
        N_local_in_c = 0;
    ...
    if (rank < p_out_r*p_out_c) {
        ...
        N_local_out_c = M; //local_size(N, nblock_out_c, rank, p_out_c, p_out_r, 'c');
    }
    else {
        N_local_out_r = 0;
        N_local_out_c = 0;
    ...
    */
    ...
    int *rank_in_c = malloc(M*sizeof(int));
    ...
    int *rank_out_c = malloc(M*sizeof(int));
    int *arr_idx_out_r = malloc(N*sizeof(int));
    int *arr_idx_out_c = malloc(M*sizeof(int));
    ...
    for ( int idx_in_r = 0 ; idx_in_r < N_local_in_r ; idx_in_r++ ) {
        ...
        for ( int idx_in_c = 0 ; idx_in_c < N_local_in_c ; idx_in_c++ ) {
            ...
            if ( rank_out == rank ) {
                ...
                // push position
                push(&gather_send[rank_out], (double) i*M + j, &amount_send[rank_out], &capacity_send[rank_out]);
            }
        }
    }
    ...
    for(int i = 0; i < mpi_size_recv ; ++i) {
        if ((i != rank) && (amount_recv[i]>0)) {
            for(int j = 0; j < amount_recv[i]; j += 2) {
                int i_global = (int) gather_recv[i][j+1] / M;
                int j_global = (int) gather_recv[i][j+1] % M;
