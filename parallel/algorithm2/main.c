...
int main(int argc, char *argv []) {
    ...
    Example:
        Data/feat_200/poly_order_10.txt --> File with number vandermonde vectors and dimensions
        Data/feat_200/rank_10.txt       --> File with number of TT-ranks and each rank
        Data/X_20000_200.txt            --> File with number of instances, dimension and all data variables
        Data/y_20000.txt                --> File with number of instances and all data outputs variables
        48                              --> Blocksize
    */
    ...
    int M; // will be broadcasted to all ranks
    double **C = NULL; // only initialized on rank 0
    
    int ite=0;
    int itemax=2; //(num_features-1)*4;
    ...
    if (rank == 0) {
        ...
        for(int i=0; i<num_features; i++) {
            ...
            zeros_3D_tensor(Grank[i], poly_order[i], Grank[i+1], G_list[i]);
        }
        ...
        double **C_transpose, **transpose_by_matrix;
        ...
        while (ite < itemax ) {
            ...
            // start alg2 timing for second loop
            if (ite == itemax) alg2_t0 = MPI_Wtime();
            ...
            // first redistribute C
            
            M = Grank[ind]*poly_order[ind]*Grank[ind+1];
            
            //MPI_Recv(&M, 1, MPI_INT, 0, ite, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            
            // send M to all other ranks
            
            for (int dest = 1; dest < size; dest++) {
                MPI_Send(&M, 1, MPI_INT, dest, ite, MPI_COMM_WORLD);
            }
            //printf("rank: %d, M: %d\n", rank, M);
            
            transpose_by_matrix = ctc(C, N, M, rank, size, block_size);
        ...
        double alg2_t = MPI_Wtime() - alg2_t0;
        // Print timing
        printf("\nTime algorithm 2 second loop  %12.8f \n\n", alg2_t);
/*
    // Save weights
    char filename[256];
    for(int i=0; i<num_features; i++) {
    ...
    */
        ...
        // since the result of the matrix transposed times matrix is reduced to rank 0, the ctc
        // returns NULL when used on other ranks
        double **nlptr = NULL;
        
        while (ite < itemax ) {
            ite = ite + 1;
            // the other ranks just help the redistribution function and ctc
            //int MPI_Recv(void *buf, int count, MPI_Datatype datatype, int source, int tag, MPI_Comm comm, MPI_Status * status)
            // receive M
            //printf("rank %d, ite %d\n", rank, ite);
            MPI_Recv(&M, 1, MPI_INT, 0, ite, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            nlptr = ctc(C, N, M, rank, size, block_size);
        }
    }
    MPI_Finalize();
    return 0;
