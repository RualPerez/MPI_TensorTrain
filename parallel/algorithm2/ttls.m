function [x, res, err]=ttls(a, b, n, r,gamma, varargin)
% [x, res, err]=ttls(a, b, n, r, gamma) or [x, res, err]=ttls(a, b, n, r, gamma, x0)
% --------------------------------------------------------------------------------------------
% Tensor Train polynomial classifier trained with least squares and
% regularization. 
%
...
while  ite < itemax  
    ...
    res(ite)=norm(C*y-b)^2/2;    % Regularized LS loss function
    err(ite)=sum(sign(C*y) ~= b)/N;                 % Sign error rate
    %[ite  res(ite)  err(ite)],    normA      
